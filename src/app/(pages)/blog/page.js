...
export default async function BlogPage(props) {
  ...
  const currentPage = rawPage ? parseInt(rawPage, 10) || 1 : 1;
            ...
            return (
              <article key={post.id} className="md:grid md:grid-cols-2 gap-8 items-center">
                ...
                <div className={`space-y-4 ${isOdd ? "md:order-1" : ""}`}>
                  <h2 className="text-xl md:text-2xl font-semibold uppercase">{post.title}</h2>

                  {/* preview  */}
                  <p className="text-sm leading-relaxed">{post.content.slice(0, 350)}...</p>

                  {/* button */}
                  <Link href={`/blog/${post.id}`} className="form-button w-full md:w-40 inline-block mt-4 text-center">
                    Read More
                  </Link>
                </div>
              </article>
          ...
          <nav className="flex items-center justify-center gap-4 mt-12 text-sm">
            ...
            <div className="flex gap-3">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNumber) => {
                ...
                const baseClasses = "h-8 w-8 flex items-center justify-center border-2 text-lg transition-all duration-200";

                const activeClasses = "border-[var(--pink)] text-[var(--pink)] bg-white/10 cursor-default pointer-events-none";

                const inactiveClasses = "border-white text-white hover:bg-white/5";

                return (
                  <Link key={pageNumber} href={`/blog?page=${pageNumber}`} aria-current={isActive ? "page" : undefined} className={`${baseClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    {pageNumber}
                  </Link>
                );
              })}
